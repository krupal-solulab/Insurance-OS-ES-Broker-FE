  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button variant="primary"><Sparkles className="h-4 w-4" />Run batch matching</Button>
          </>
        }
      />
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="w-72"><SearchBar placeholder="Search by insured, agent, id…" /></div>
      </div>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex items-center gap-2 text-[11px] text-muted-foreground">
                    <span>{row.agency}</span>·<span>{row.state}</span>·<span className="font-mono">{row.premium}</span>
                  </div>
                  <div className="mt-1.5 flex items-center gap-2">
                    <Chip tone={row.appetite === "Strong fit" ? "success" : row.appetite === "Marginal fit" ? "warn" : "danger"}>{row.appetite}</Chip>
        ...
        <div className="space-y-5">
          <Panel>
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="primary">Proceed to package <ArrowRight className="h-4 w-4" /></Button>
              </div>
            </div>

            <div className="mt-5 grid gap-3 md:grid-cols-3">
              <MetricTile label="Best-fit score" value={s.score.toString()} sub={`Top market · ${s.topMarket}`} tone={s.score >= 80 ? "success" : "warn"} />
              <MetricTile label="Panel fit" value={s.appetite} sub={`${s.marketsInAppetite} of 5 markets in appetite`} tone={s.appetite === "Strong fit" ? "success" : "warn"} />
              <MetricTile label="Recommendation" value={s.recommendation} sub="AI + broker co-sign" tone={s.recommendation === "Proceed to market" ? "success" : s.recommendation === "No market" ? "danger" : "warn"} />
            </div>
          </Panel>
          ...
          <ProcessAnim
            steps={[
              ...
              { label: "Per-carrier missing-info identified · recommendation drafted", kind: "matching" },
            ]}
          />

          <Panel>
            <Tabs tabs={["Documents", "Carrier ranking", "Match rules", "AI recommendation", "Activity"]} value={tab} onChange={setTab} />
            <div className="mt-5">
              {tab === "Documents" && <DocumentsTab />}
              {tab === "Carrier ranking" && <CarrierRankingTab />}
        ...
        <div className="relative overflow-hidden rounded-lg border border-border bg-paper">
          <div className="grid grid-cols-2 gap-6 p-6 font-mono text-[11px] text-ink-soft">
            ...
            <div>
              ...
              <div className="rounded bg-accent/15 px-1 py-0.5 text-foreground">Kinsale Insurance</div>
...
function CarrierRankingTab() {
  return (
    <div className="grid gap-4 lg:grid-cols-[1.3fr_1fr]">
      <div>
        <div className="mb-2 flex items-center justify-between">
          <div className="text-xs font-medium">Ranked carrier panel · 4 of 5 in appetite</div>
          <FoundationBadge kind="matching" />
        </div>
        <ul className="space-y-2">
          {submissionMarkets.map((m, i) => (
            <li key={m.carrier} className={`rounded-lg border p-3 text-sm ${m.fit === "Out of appetite" ? "border-border opacity-60" : "border-border"}`}>
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <span className="grid h-6 w-6 place-items-center rounded-full bg-secondary text-[11px] font-mono">{i + 1}</span>
                  <span className="font-medium">{m.carrier}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Chip tone={m.fit === "Strong fit" ? "success" : m.fit === "Marginal fit" ? "warn" : "danger"}>{m.fit}</Chip>
              ...
              {m.missingInfo.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {m.missingInfo.map((mi) => (
                    <Chip key={mi} tone="warn"><AlertTriangle className="h-2.5 w-2.5" />{mi}</Chip>
                  ))}
                </div>
              )}
            </li>
      ...
      <div>
        ...
        <div className="space-y-2 text-sm">
          <Consistency label="Class fit" ok detail="Cold storage / refrigerated warehousing is an actively sought class for the top 2 carriers" />
          <Consistency label="Capacity fit" ok detail="$42.8M TIV sits well inside Kinsale's $50M treaty and James River's combined $50M layer" />
          <Consistency label="Loss history" ok detail="38% 5yr loss ratio clears every panel carrier's threshold" />
          <Consistency label="Missing info" warn detail="James River and Ategrity need supplemental docs before they'll firm a quote — routed to Package Assembly" />
          <Consistency label="Palomar Specialty" detail="Class-excluded per their current appetite profile — excluded automatically, not scored" />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        <div className="text-xs font-medium">Match rules · top carrier (Kinsale Insurance) · 5 of 6 pass</div>
        <FoundationBadge kind="matching" />
      </div>
    ...
    <div className="grid gap-4 md:grid-cols-3">
      <div className="rounded-xl border-2 border-success/40 bg-success/5 p-4 md:col-span-2">
        ...
        <p className="mt-3 text-sm text-foreground">
          ...
          92% sprinklered TIV, and revenue growth of 11% CAGR. Four of five panel carriers are in appetite;
          Kinsale Insurance is the strongest fit with no outstanding missing info. Recommend packaging for
          Kinsale, James River, and Ategrity in parallel to maximize competitive tension.
        </p>
        <div className="mt-4 grid gap-2 text-xs text-muted-foreground">
          <div><b className="text-foreground">Suggested package priority:</b> Kinsale Insurance (ready now) → James River Insurance (needs sprinkler inspection) → Ategrity Specialty (needs loss run addendum + monitoring cert).</div>
          <div><b className="text-foreground">Suggested subjectivities to pre-empt:</b> updated sprinkler inspection, confirmation of continuous refrigeration monitoring.</div>
        </div>
      </div>
      <div className="rounded-xl border border-border p-4">
        <div className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Broker decision</div>
        <div className="mt-3 space-y-2 text-sm">
          <Button variant="primary" className="w-full justify-center">Approve recommendation</Button>
          <Button variant="secondary" className="w-full justify-center">Modify carrier selection</Button>
          <Button variant="secondary" className="w-full justify-center">Send to peer review</Button>
          <Button variant="danger" className="w-full justify-center">Override — no market</Button>
        </div>
...
const packageCarriers = [
  { carrier: "Kinsale Insurance", completeness: 100, status: "Ready to send" as const, missing: [] as string[] },
  { carrier: "James River Insurance", completeness: 83, status: "Missing 1 doc" as const, missing: ["Updated sprinkler inspection"] },
  { carrier: "Ategrity Specialty", completeness: 67, status: "Missing 2 docs" as const, missing: ["5-yr loss run addendum", "Refrigeration monitoring cert"] },
  { carrier: "Markel", completeness: 50, status: "Sent" as const, missing: [] },
];

export function PackageAssembly() {
  const [sel, setSel] = useState(packageCarriers[0].carrier);
  const c = packageCarriers.find((x) => x.carrier === sel)!;
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageHeader
        eyebrow="Workflow 02"
        title="Submission Package Assembly"
        description="Turn a carrier selection from Market Matching into a ready-to-send package — completeness check, bounded auto-fill, cover letter drafted per carrier."
        actions={<Button variant="primary"><Send className="h-4 w-4" />Send ready packages</Button>}
      />

      <div className="mb-5">
        <ProcessAnim
          steps={[
            { label: "Carrier selection received from Market Matching · 4 markets", kind: "matching" },
            { label: "Document completeness checked per carrier", kind: "extraction" },
            { label: "Supplemental forms auto-filled — extracted fields only, nothing invented", kind: "extraction" },
            { label: "Cover letter drafted, tailored per carrier", kind: "matching" },
          ]}
        />
      </div>

      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.6fr)]">
        <Panel title="Packages · Palmetto Cold Storage LLC" subtitle="SUB-24019 · 4 carriers selected">
          <ul className="divide-y divide-border">
            {packageCarriers.map((p) => (
              <button key={p.carrier} onClick={() => setSel(p.carrier)} className={`flex w-full flex-col gap-2 py-3 text-left ${sel === p.carrier ? "bg-secondary/50" : "hover:bg-secondary/30"}`}>
                <div className="flex items-center justify-between text-sm">
                  <span className="font-medium">{p.carrier}</span>
                  <Chip tone={p.status === "Ready to send" ? "success" : p.status === "Sent" ? "accent" : "warn"}>{p.status}</Chip>
                </div>
                <div className="h-1.5 overflow-hidden rounded-full bg-secondary">
                  <div className={`h-full ${p.completeness === 100 ? "bg-success" : "bg-accent"}`} style={{ width: `${p.completeness}%` }} />
                </div>
                <div className="text-[11px] text-muted-foreground">{p.completeness}% document-complete</div>
              </button>
            ))}
          </ul>
        </Panel>
        ...
        <div className="space-y-5">
          <Panel title={`Package · ${c.carrier}`} actions={<FoundationBadge kind="extraction" />}>
            <div className="mb-3 text-xs font-medium">Document completeness checklist</div>
            <ul className="divide-y divide-border rounded-lg border border-border text-sm">
              {["ACORD 125", "ACORD 140", "Schedule of Values", "5-yr Loss Run", "Financials FY24", "Carrier supplemental form"].map((doc) => {
                const isMissing = c.missing.some((m) => doc.toLowerCase().includes(m.split(" ")[0].toLowerCase()) || m.toLowerCase().includes(doc.split(" ")[0].toLowerCase()));
                return (
                  <li key={doc} className="flex items-center gap-3 p-3">
                    {isMissing ? <AlertTriangle className="h-4 w-4 shrink-0 text-warn" /> : <CheckCircle2 className="h-4 w-4 shrink-0 text-success" />}
                    <span className="flex-1">{doc}</span>
                    {isMissing && <Chip tone="warn">Missing</Chip>}
                  </li>
                );
              })}
            </ul>
          ...
          <Panel title="Auto-filled supplemental form" subtitle="Bounded auto-fill — extracted fields only, never invented">
            <div className="grid gap-3 rounded-lg border border-border p-4 text-sm sm:grid-cols-2">
              {[
                ["Named insured", "Palmetto Cold Storage LLC"],
                ["FEIN", "58-1298347"],
                ["Refrigeration capacity", "14 locations · 92% sprinklered"],
                ["Requested effective date", "02/12/2026"],
                ["Prior carrier / premium", "Kinsale Insurance · $168,900"],
                ["5yr loss ratio", "38%"],
              ].map(([label, value]) => (
                <div key={label}>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div>
                  <div className="rounded bg-accent/15 px-1.5 py-0.5 text-foreground">{value}</div>
                </div>
              ))}
            </div>
          </Panel>

          <Panel title="Cover letter draft" actions={<Chip tone="accent">Tone: Confident</Chip>}>
            <textarea
              rows={5}
              defaultValue={`Hi team — submitting Palmetto Cold Storage LLC (Jacksonville, FL) for your review. FL cold-storage risk, $42.8M TIV across 14 locations, 92% sprinklered, clean 38% 5-year loss ratio. Full package attached: ACORD 125/140, SOV, 5yr loss run, FY24 financials. Effective 02/12/2026 — happy to jump on a call this week.`}
              className="w-full resize-none rounded-lg border border-border bg-background p-3 text-sm outline-none"
            />
            <div className="mt-3 flex justify-end gap-2">
              <Button variant="ghost"><Sparkles className="h-4 w-4" />Regenerate</Button>
              <Button variant="secondary">Edit</Button>
              <Button variant="primary">Send package <Send className="h-3.5 w-3.5" /></Button>
            </div>
          </Panel>
        </div>
      </div>
...
export function RetailAgentCopilot() {
  const threads = [
    { id: 1, agent: "Ana Ruiz · Marsh Southeast", subject: "Palmetto Cold Storage — market Q&A", unread: true, snippet: "Confirming spoilage sub-limit at $500k…", when: "8:42 AM" },
    { id: 2, agent: "Michael Chen · HUB International", subject: "Ridgeline Contractors — missing SOV", unread: true, snippet: "Attached is the schedule you requested…", when: "8:11 AM" },
    { id: 3, agent: "Priya Natarajan · Alliant", subject: "Bayou Marine — placement update", unread: false, snippet: "Any early read on markets?…", when: "Yesterday" },
    { id: 4, agent: "Jordan Blake · Gallagher", subject: "Highline Hospitality — no-market notice", unread: false, snippet: "Understood, thanks for the quick turn…", when: "Jan 09" },
  ];
  const [active, setActive] = useState(1);
  const drafts = [
    { title: "Quote summary", tone: "Warm", body: "Hi Ana, great news on Palmetto Cold Storage — Kinsale Insurance has come back with indicated terms of $187,400, including the $500k spoilage sub-limit you asked about. Two more markets are still reviewing. Happy to walk through it on a call." },
    { title: "Missing info request", tone: "Direct", body: "Hi Michael, to keep Ridgeline Contractors moving with our panel we still need a current SOV and 5-year loss run. Would you be able to send those today so we can get markets responding by Wednesday?" },
    { title: "No-market notice", tone: "Considerate", body: "Hi Jordan, thanks for thinking of us on Highline Hospitality. After running it across our panel, this one falls outside every carrier's current appetite for the liquor liability exposure — we'd love to see the rest of your submissions this quarter." },
  ];
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageHeader
      ...
      <div className="grid gap-0 overflow-hidden rounded-2xl border border-border bg-background lg:grid-cols-[280px_minmax(0,1fr)_320px]">
        ...
        <div className="border-r border-border">
          <div className="border-b border-border p-3"><SearchBar placeholder="Search retail agents…" /></div>
          <ul className="divide-y divide-border">
            {threads.map((t) => (
              <button
                key={t.id}
        ...
        <div className="flex min-h-[520px] flex-col">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto p-4 text-sm">
            <Bubble from="ana" name="Ana Ruiz" at="8:12 AM">
              Hi team — sending over Palmetto Cold Storage. Two new locations in Jacksonville. Insured would love to
              confirm the spoilage sub-limit at $500k. Any early read on markets?
            </Bubble>
            <Bubble from="you" name="Coverline AI · drafted" at="8:14 AM" ai>
              Thanks Ana — reviewing now, we're ranking it across our panel and should have indicated terms within
              the hour. Palmetto's loss history and sprinkler profile look strong across every carrier we'd send it
              to. We can confirm the $500k spoilage sub-limit is a standard ask.
            ...
            <Bubble from="ana" name="Ana Ruiz" at="8:31 AM">
              Perfect, thank you. Also — will the carrier be disclosed before we get terms, or after?
            </Bubble>
            <Bubble from="you" name="Coverline AI · drafted" at="8:42 AM" ai>
              After — per our standard process we don't disclose the carrier name until you've confirmed broker of
              record on this account. Once that's on file we'll name Kinsale Insurance directly in the terms letter.
              <div className="mt-2 flex items-center gap-2 text-[10px] text-muted-foreground">
                <Lock className="h-3 w-3" /> Carrier-name disclosure gate — compliance-controlled, cannot be bypassed from this draft.
              </div>
              <div className="mt-2 flex gap-2 text-xs">
                <Button variant="ghost">Regenerate</Button>
                <Button variant="secondary">Edit</Button>
...
function Bubble({ children, from, name, at, ai }: { children: ReactNode; from: "ana" | "you"; name: string; at: string; ai?: boolean }) {
  ...
  return (
    <div className={`flex gap-3 ${isYou ? "justify-end" : ""}`}>
      ...
      {isYou && <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-foreground text-xs font-medium text-background">SD</div>}
    </div>
  );
}
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageHeader
        ...
        actions={<Button variant="primary"><Send className="h-4 w-4" />Send comparison to agent</Button>}
      />

      <div className="mb-5">
        <ProcessAnim
          steps={[
            { label: "3 carrier response emails ingested · Highline Hospitality Group", kind: "extraction" },
            { label: "Premium, deductible, and limit terms normalized to one schema", kind: "extraction" },
            { label: "Subjectivities classified by materiality (Standard / Material / Deal-breaker)", kind: "matching" },
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <MetricTile label="Markets responded" value={`${responded} of ${quotes.length}`} sub="1 declined — Ategrity Specialty" tone="success" />
        <MetricTile label="Premium spread" value="$27,500" sub="$421,000 – $448,500" tone="warn" />
        <MetricTile label="Recommended carrier" value="James River" sub="Lowest premium, standard subjectivities only" tone="success" />
      </div>

      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        <Panel title="Normalized quote comparison" subtitle="Highline Hospitality Group · SUB-24016" actions={<FoundationBadge kind="matching" />}>
              ...
              <tbody className="divide-y divide-border">
                {quotes.map((q) => (
                  <tr key={q.carrier} className={q.status === "Declined" ? "opacity-50" : ""}>
                    ...
                    <td className="py-2.5 pl-4 text-xs text-muted-foreground">{q.subjectivities.length ? q.subjectivities.join("; ") : "—"}</td>
                    <td className="py-2.5 pl-4"><Chip tone={q.materiality === "Deal-breaker" ? "danger" : q.materiality === "Material" ? "warn" : "success"}>{q.materiality}</Chip></td>
                    <td className="py-2.5 pl-4 text-xs">{q.validUntil}</td>
                    <td className="py-2.5 pl-4"><Chip tone={q.status === "Quoted" ? "accent" : q.status === "Bound" ? "success" : "danger"}>{q.status}</Chip></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Panel title="AI recommendation">
          <div className="rounded-xl border-2 border-success/40 bg-success/5 p-4">
            ...
            <p className="mt-2 text-sm">
              James River is $27,500 lower with only standard subjectivities (signed application, liquor license
              copy). Markel's price is close but adds two material subjectivities — a 4-year loss run pull and a
              written security plan — that would add turnaround risk before the effective date.
            </p>
          </div>
          <div className="mt-4 space-y-2 text-sm">
            <Button variant="primary" className="w-full justify-center">Present to retail agent</Button>
            <Button variant="secondary" className="w-full justify-center">Request revised terms</Button>
            <Button variant="danger" className="w-full justify-center">Decline all — remarket</Button>
          </div>
        </Panel>
      </div>
    </div>
...
export function BinderIssuance() {
  ...
  const steps = [
    ...
    { label: "Bind request sent to carrier", done: true, note: "Sent with signed application + subjectivity tracker" },
    { label: "Subjectivity clearance", done: b.subjectivitiesCleared === b.subjectivitiesTotal, active: b.subjectivitiesCleared < b.subjectivitiesTotal, note: `${b.subjectivitiesCleared} of ${b.subjectivitiesTotal} cleared` },
    { label: "Carrier bind confirmation reconciled", done: b.status === "Bound — awaiting policy" || b.status === "Issued", active: b.status === "Ready to bind", note: "Checked line-by-line against agreed terms — 0 discrepancies" },
    { label: "Issued policy reconciled against binder", done: b.status === "Issued", active: b.status === "Bound — awaiting policy", note: "Declarations page compared to bound terms" },
    { label: "Retail agent & insured notified", done: b.status === "Issued", note: "One-click notification" },
  ];
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageHeader
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
          ...
          <ul className="divide-y divide-border">
            {binders.map((r) => (
              <button key={r.id} onClick={() => setSel(r.id)} className={`w-full py-3 text-left ${sel === r.id ? "bg-secondary/50" : "hover:bg-secondary/30"}`}>
                ...
                <div className="text-[11px] text-muted-foreground">{r.id} · {r.carrier}</div>
                <div className="mt-1 flex items-center gap-2">
                  <Chip tone={r.status === "Issued" ? "success" : r.status === "Ready to bind" ? "accent" : "warn"}>{r.status}</Chip>
        ...
        <div className="space-y-5">
          <Panel title={`${b.insured} · bind workflow`} subtitle={`${b.id} · ${b.carrier}`} actions={<FoundationBadge kind="matching" />}>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <Panel title="AI bind recommendation">
              <div className="rounded-xl border-2 border-success/40 bg-success/5 p-4">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-5 w-5 text-success" />
                  <div className="font-serif text-lg">{b.status === "Ready to bind" ? "Ready to bind" : b.status}</div>
                </div>
                <p className="mt-2 text-sm">
                  {b.subjectivitiesCleared} of {b.subjectivitiesTotal} subjectivities satisfied. Premium {b.premium},
                  effective {b.effective}. Never treats a carrier document as authoritative without reconciling it
            ...
            <Panel title="Policy issuance">
              <div className="space-y-2 text-sm">
                <ProgressRow label="Bind confirmation reconciled" pct={b.status === "Subjectivities open" ? 40 : 100} />
                <ProgressRow label="Policy jacket assembly" pct={b.status === "Issued" ? 100 : 70} />
                <ProgressRow label="Declarations reconciliation" pct={b.status === "Issued" ? 100 : b.status === "Bound — awaiting policy" ? 55 : 0} />
                <ProgressRow label="Surplus lines filing" pct={b.status === "Issued" ? 100 : 30} />
                <ProgressRow label="PAS / book-of-record write-back" pct={b.status === "Issued" ? 100 : 0} />
              </div>
            </Panel>
          </div>
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
            ...
            {midTermChanges.map((r) => (
              <button key={r.id} onClick={() => setSel(r.id)} className={`w-full py-3 text-left ${sel === r.id ? "bg-secondary/50" : "hover:bg-secondary/30"}`}>
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  <Chip tone={r.materiality === "Complex" ? "danger" : r.materiality === "Material" ? "warn" : "success"}>{r.materiality}</Chip>
                </div>
              </button>
            ))}
          </ul>
        ...
        <div className="space-y-5">
          <Panel>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="text-[11px] font-mono text-muted-foreground">{e.id} · Policy {e.policy} · {e.carrier}</div>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            <Panel title="Materiality classification" actions={<Chip tone={e.materiality === "Complex" ? "danger" : e.materiality === "Material" ? "warn" : "success"}>{e.materiality}</Chip>}>
            ...
            <Panel title="Appetite recheck" actions={<FoundationBadge kind="matching" />}>
              <ul className="space-y-2 text-sm">
                <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-success" />New location within {e.carrier}'s permitted state list</li>
                <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-success" />TIV still under carrier's treaty cap</li>
                <li className="flex items-center gap-2"><Chip tone={e.appetiteRecheck === "Still in appetite" ? "success" : e.appetiteRecheck === "Marginal — review" ? "warn" : "danger"}>{e.appetiteRecheck}</Chip></li>
              </ul>
            </Panel>
            <Panel title="Item-level reconciliation">
              <ul className="space-y-2 text-sm">
                <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-success" />{e.type.split(" + ")[0]} — reconciled</li>
                {e.type.includes("+") && <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-success" />{e.type.split(" + ")[1]} — reconciled independently</li>}
                <li className="flex items-center gap-2"><Info className="h-4 w-4 text-muted-foreground" />Combined premium impact: {e.impact}</li>
              </ul>
            </Panel>
          </div>
        </div>
...
export function RenewalRemarketing() {
  ...
  const triggerTone = r.trigger === "Renew as-is" ? "success" : r.trigger === "Monitor" ? "neutral" : r.trigger === "Remarket — competitive check" ? "warn" : "danger";
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageHeader
        eyebrow="Workflow 07"
        ...
        actions={<Button variant="primary"><Sparkles className="h-4 w-4" />Run remarket review</Button>}
      />
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,2.1fr)]">
          ...
          <ul className="divide-y divide-border">
            {remarketing.map((row) => (
              ...
              >
                ...
                <Chip tone={row.trigger === "Renew as-is" ? "success" : row.trigger === "Monitor" ? "neutral" : row.trigger === "Remarket — competitive check" ? "warn" : "danger"}>{row.trigger}</Chip>
              </button>
            ))}
          </ul>
        </Panel>
        ...
        <div className="space-y-5">
          <Panel>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="text-[11px] font-mono text-muted-foreground">{r.id} · Incumbent {r.incumbentCarrier}</div>
          ...
          <Panel title="Side-by-side comparison" subtitle="Prior term vs. renewal submission" actions={<FoundationBadge kind="extraction" />}>
            <div className="grid grid-cols-3 gap-0 overflow-hidden rounded-lg border border-border text-sm">
              ...
              <Row label="Incumbent responsiveness" prior="—" now={r.incumbentResponsive ? "Responsive" : "Non-responsive — 12+ business days"} />
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <Panel title="AI remarket recommendation" actions={<Chip tone={triggerTone}>{r.trigger}</Chip>}>
              <p className="text-sm text-foreground">
                ...
                {r.trigger === "Remarket — competitive check" && "Rate change and exposure growth justify a light competitive check against 1–2 alternate markets, without disrupting the incumbent relationship."}
                {r.trigger === "Remarket — active shop" && "Incumbent has gone non-responsive and loss trend has deteriorated — recommend an active shop across the full panel before the expiring date."}
              </p>
              <ul className="mt-3 space-y-1 text-xs text-muted-foreground">
                <li>· Flag: {r.flag}</li>
                <li>· Remarketing-history weighting: this account has been remarketed 0 times in the last 3 years</li>
              </ul>
            </Panel>
            <Panel title="Retail agent context">
              <div className="text-sm">
                <div className="font-medium">Ana Ruiz · Marsh Southeast</div>
                <div className="text-[11px] text-muted-foreground">Agent for 4 years · 28 bound placements</div>
              </div>
              <div className="mt-3 rounded-lg border border-border bg-secondary/40 p-3 text-sm text-ink-soft">
                ...
                confirmation and a 15-day extension if bind slips past the expiring date."
              </div>
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageHeader
        ...
        actions={<Button variant="primary" disabled={!canGenerate}><FileSearch className="h-4 w-4" />Generate documentation</Button>}
      />
      ...
      <div className="mb-5 flex items-start gap-3 rounded-xl border border-border bg-secondary/40 p-4 text-sm">
        ...
        <div>
          ...
          <div className="text-[11px] text-muted-foreground">This workflow will not generate surplus-lines documentation unless every required declination has sufficient written evidence on file, or the state is confirmed exempt for this class. No exceptions.</div>
        </div>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.6fr)]">
          ...
          <ul className="divide-y divide-border">
            {diligentSearch.map((r) => (
              <button key={r.id} onClick={() => setSel(r.id)} className={`w-full py-3 text-left ${sel === r.id ? "bg-secondary/50" : "hover:bg-secondary/30"}`}>
                ...
                <div className="text-[11px] text-muted-foreground">{r.id} · {r.declinationsOnFile} of {r.requiredDeclinations} declinations on file</div>
                <div className="mt-1"><Chip tone={r.status === "Filed" || r.status === "Exempt" ? "success" : r.status === "Ready to file" ? "accent" : "warn"}>{r.status}</Chip></div>
              </button>
            ))}
          </ul>
        </Panel>
        ...
        <div className="space-y-5">
          <Panel>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="text-[11px] font-mono text-muted-foreground">{d.id} · {d.state}</div>
                <h2 className="mt-1 font-serif text-2xl">{d.insured}</h2>
              </div>
              <Chip tone={d.status === "Filed" || d.status === "Exempt" ? "success" : d.status === "Ready to file" ? "accent" : "warn"}>{d.status}</Chip>
            </div>
          </Panel>
          ...
          <Panel title="Per-state requirement" actions={<FoundationBadge kind="matching" />}>
            {d.status === "Exempt" ? (
              <p className="text-sm text-foreground">
                {d.state} does not require diligent-search documentation for this class of business at this TIV —
                confirmed exempt against the current per-state requirement reference. No declinations needed.
              </p>
            ) : (
              <p className="text-sm text-foreground">
                {d.state} requires <b>{d.requiredDeclinations} declinations</b> from admitted markets before this
                risk can be placed non-admitted. {d.declinationsOnFile} of {d.requiredDeclinations} are on file with
                ...
                {Array.from({ length: d.requiredDeclinations }).map((_, i) => {
                  ...
                  return (
                    <li key={i} className="flex items-start gap-3 p-3 text-sm">
                      {has ? (sufficient ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-success" /> : <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-warn" />) : <XCircle className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />}
                      <div className="flex-1">
                        <div className="font-medium">Declination {i + 1}</div>
                        <div className="text-[11px] text-muted-foreground">
                          {has ? (sufficient ? "Written evidence on file — sufficient" : "On file but evidence insufficient — needs a dated written declination, not a verbal note") : "Not yet received"}
                        </div>
                      </div>
                      {!has && <Button variant="secondary" className="!py-1 !text-xs">Chase market</Button>}
                    </li>
                  );
                })}
              </ul>
          ...
          <Panel title="Compliant documentation">
            {canGenerate ? (
              <div className="rounded-xl border-2 border-success/40 bg-success/5 p-4">
                <div className="flex items-center gap-2"><CheckCircle2 className="h-5 w-5 text-success" /><div className="font-serif text-lg">Ready to generate</div></div>
                <p className="mt-2 text-sm">Surplus lines affidavit and diligent-search record will cite every declination on file with source and date. Nothing is generated from summary or verbal evidence.</p>
                <div className="mt-3"><Button variant="primary">Generate affidavit <FileSearch className="h-4 w-4" /></Button></div>
              </div>
            ) : (
              <div className="rounded-xl border-2 border-warn/40 bg-warn/5 p-4">
                <div className="flex items-center gap-2"><Ban className="h-5 w-5 text-warn" /><div className="font-serif text-lg">Generation blocked</div></div>
                <p className="mt-2 text-sm">Evidence is insufficient for at least one required declination. Chase the outstanding market(s) before documentation can be generated — this gate cannot be overridden from this screen.</p>
              </div>
            )}
          </Panel>
        </div>
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageHeader
        ...
        actions={<Button variant="primary"><Download className="h-4 w-4" />Export signal log</Button>}
      />
      ...
      <div className="mb-5 flex items-start gap-3 rounded-xl border border-border bg-secondary/40 p-4 text-sm">
        ...
        <div>
          ...
          <div className="text-[11px] text-muted-foreground">This workflow only aggregates, suggests, and auto-updates exactly two metadata fields — carrier fit score and last-signal date. Treat that as binding, not a starting point to expand from.</div>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <div className="mt-1 flex items-center gap-1.5">
                      {s.signal.includes("expanding") ? <TrendingUp className="h-3.5 w-3.5 text-success" /> : s.signal.includes("tightening") ? <TrendingDown className="h-3.5 w-3.5 text-destructive" /> : <Info className="h-3.5 w-3.5 text-muted-foreground" />}
                      <span>{s.signal}</span>
                    </div>
                    <div className="mt-1 text-[11px] text-muted-foreground">Evidence: {s.evidence}</div>
                    <div className="mt-1 text-[11px]"><b>Suggested:</b> {s.suggestedAction}</div>
                  </div>
                  <div className="flex shrink-0 flex-col items-end gap-2">
                    <Chip tone={s.status === "Approved" ? "success" : s.status === "Dismissed" ? "danger" : "warn"}>{s.status}</Chip>
                    {s.status === "Pending review" && (
                      <div className="flex gap-1">
                        <Button variant="secondary" className="!py-1 !text-xs">Dismiss</Button>
                        <Button variant="primary" className="!py-1 !text-xs">Approve</Button>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <Panel title="Suppressed from auto-signal" subtitle="Low submission volume — human review required">
          <ul className="divide-y divide-border text-sm">
            <li className="flex items-center justify-between py-3"><span>Ategrity Specialty · Marine terminal ops</span><Chip>3 subs / 90d</Chip></li>
            <li className="flex items-center justify-between py-3"><span>Palomar Specialty · Environmental</span><Chip>1 sub / 90d</Chip></li>
          </ul>
          <div className="mt-3 text-[11px] text-muted-foreground">Volume below the confidence threshold — signals here are logged but never auto-suggested.</div>
        </Panel>
      </div>

      <div className="mt-6">
        <Panel title="Decision timeline" subtitle="Every AI and human decision, fully auditable" actions={<FoundationBadge kind="matching" />}>
          <ul className="divide-y divide-border">
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">
        <Panel className="lg:col-span-2" title="Submission → bound funnel" subtitle="Trailing 30 days · 104 of 274 submissions bound (38.0%)">
            ...
            {funnel.map((f) => (
              <div key={f.stage} className="flex items-center gap-3 text-sm">
                ...
                <div className="h-6 flex-1 overflow-hidden rounded-md bg-secondary">
                  <div className="flex h-full items-center justify-end bg-accent px-2 text-[10px] font-mono text-accent-foreground" style={{ width: `${(f.count / max) * 100}%` }}>
                    {f.count}
                  </div>
                </div>
              </div>
          ...
          <div className="mt-3 text-[10px] text-muted-foreground">Low-volume annotation: state- and class-level cuts below 10 submissions/mo are flagged, not silently averaged in.</div>
        </Panel>
            ...
            {stateMix.map((s) => {
              ...
              return (
                <div key={s.state} className="flex items-center gap-3 text-xs">
                  ...
                  <div className="h-2 flex-1 overflow-hidden rounded-full bg-secondary"><div className="h-full bg-foreground" style={{ width: `${(s.premium / max2) * 100}%` }} /></div>
        ...
        <Panel title="Remarketing value report" actions={<FoundationBadge kind="matching" />}>
          <div className="space-y-3 text-sm">
            <div className="rounded-lg border border-border p-3">
              <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Accounts remarketed, trailing 12mo</div>
              <div className="mt-1 font-serif text-2xl">18</div>
            </div>
            <div className="rounded-lg border border-border p-3">
              <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Premium improvement captured</div>
              <div className="mt-1 font-serif text-2xl">$212k</div>
              <div className="text-[11px] text-muted-foreground">vs. incumbent's initial renewal indication</div>
            </div>
            <div className="rounded-lg border border-border p-3">
              <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Retention on remarketed accounts</div>
              <div className="mt-1 font-serif text-2xl">78%</div>
              <div className="text-[11px] text-muted-foreground">Moved to a new market but stayed on the book</div>
            </div>
          </div>
        </Panel>
      </div>
      ...
      <div className="mt-6">
        <Panel title="Executive AI insight" actions={<FoundationBadge kind="matching" />}>
          <p className="text-sm">
            Placement cycle is averaging 6.4 days against a 2–3 day target — Ategrity Specialty's 9.4-day turnaround
            is the single largest drag on the panel average. Kinsale Insurance is the strongest overall channel
            (42% hit ratio, 4.1-day turnaround) — recommend routing more marginal-fit cold-storage and habitational
            risk to them first. Watch item: CO contractor excess hit ratio has fallen 6pp since the last appetite
            signal from Markel.
          </p>
          <div className="mt-3 flex gap-2">
