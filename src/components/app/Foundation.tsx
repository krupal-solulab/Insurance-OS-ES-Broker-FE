...
const matchingCaps = [
  { title: "Carrier appetite profiles", desc: "A manually-curated appetite schema per carrier — class, capacity, state, exclusions." },
  { title: "Hard-exclusion rules", desc: "Instant disqualification for out-of-appetite class, state, or TIV — no scoring wasted." },
  { title: "Soft-scoring ranking", desc: "Every non-excluded carrier ranked by composite fit score, explainable factor by factor." },
  { title: "Per-carrier missing-info detection", desc: "Each carrier's supplemental requirements checked against what's already been extracted." },
  { title: "AI explanations", desc: "Every ranking includes a plain-English rationale + citations." },
  { title: "Human review", desc: "Every recommendation routes to a broker before it becomes external action." },
  { title: "Decision logging", desc: "Immutable log of every AI and human decision, exportable." },
  { title: "Audit trail", desc: "Rule-level explanations, override rationales, and citations tied to each submission." },
];
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
        <Panel title="What it does">
          <p className="text-sm text-ink-soft">
            Extraction Core reads the messy inputs of the E&S world — retail-agent emails, ACORD forms, SOVs, loss
            runs, financials, carrier quote/declination emails, bind confirmations — and turns them into a
            consistent, cited data model that every downstream workflow can trust.
          </p>
      ...
      <div className="mt-5">
        <Panel title="Reused by">
          <div className="flex flex-wrap gap-2">
            {["Market Matching", "Package Assembly", "Agent Copilot", "Quote Comparison", "Binder & Issuance", "Endorsement Processing", "Renewal Remarketing", "Diligent Search"].map((w) => (
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
        ...
        <Panel title="Carrier appetite profiles" subtitle="Manually-curated · broker-tunable">
          <ul className="divide-y divide-border">
            {[
              { rule: "Kinsale — cold storage / refrigerated warehousing", version: "v3.2", eval: "2,148" },
              { rule: "James River — liquor liability, hospitality", version: "v2.8", eval: "1,984" },
              ...
              { rule: "Berkley Specialty — senior care professional liab.", version: "v4.0", eval: "1,842" },
            ].map((r) => (
        ...
        <Panel className="min-h-[520px]">
          <div className="space-y-4 text-sm">
            <div className="rounded-xl border border-border bg-secondary/40 p-4">
              ...
              <p>Good morning Sam. You have 12 submissions awaiting review, 7 renewals up for remarket review in the next 30 days, and 2 diligent-search records waiting on declination evidence. What would you like to work on?</p>
            </div>
            <div className="flex justify-end">
              <div className="max-w-[75%] rounded-xl border border-accent/25 bg-accent/5 p-3">Summarize submission SUB-24019 and tell me what's missing before I can package it.</div>
            </div>
            <div className="rounded-xl border border-border bg-secondary/40 p-4">
              <div className="mb-1 flex items-center gap-2 text-[11px] font-medium uppercase tracking-wider text-muted-foreground"><Sparkles className="h-3 w-3 text-accent" /> Coverline AI</div>
              <p>
                <b>SUB-24019 · Palmetto Cold Storage LLC</b> — FL warehousing risk from Marsh Southeast. TIV
                $42.8M across 14 locations, sprinklered 92%. Five-year loss ratio 38%. Ranked against the panel,
                4 of 5 carriers are in appetite — Kinsale Insurance is the top fit at score 82, ready to package
                with no missing info.
              </p>
              <p className="mt-2">James River and Ategrity Specialty need a fresh sprinkler inspection and a loss run addendum, respectively, before they'll firm a quote.</p>
          ...
          <div className="mt-6 rounded-xl border border-border p-2">
            <textarea rows={3} value={msg} onChange={(e) => setMsg(e.target.value)} placeholder="Ask about a submission, carrier panel, retail agent, or the pipeline…" className="w-full resize-none bg-transparent p-2 text-sm outline-none" />
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">
        <Panel title="Placement cycle trend · rolling 12 mo" className="lg:col-span-2">
          <svg viewBox="0 0 600 220" className="h-56 w-full">
            ...
            <line x1="0" y1="50" x2="600" y2="50" stroke="oklch(0.86 0.01 85)" strokeDasharray="4 4" />
            <text x="4" y="48" fontSize="10" fill="oklch(0.48 0.015 250)">Target 2–3 days</text>
          </svg>
        </Panel>
      ...
      <div className="space-y-5">
        ...
        <Panel title="Notifications">
          <div className="space-y-2 text-sm">
            {["New submissions from priority retail agents", "Remarket reviews due in 30 days", "AI recommendation confidence below 80%", "Diligent search evidence missing", "Override submitted"].map((n) => (
